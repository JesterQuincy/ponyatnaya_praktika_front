        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="flex gap-[20px] font-montserrat">
            <label className="flex gap-[5px] items-center">
              <input type="radio" value="client" checked={watchType === 'client'} {...register('type')} />
              <span>Встреча с клиентом</span>
            </label>
            <label className="flex gap-[5px] items-center">
              <input type="radio" value="other" checked={watchType === 'other'} {...register('type')} />
              <span>Иное</span>
            </label>
          </div>
          ...
          {watchType === 'client' ? (
            ...
            <div className="pt-[25px]">
              <input className={styles.input} placeholder="Введите название" {...register('meetingName')} />
            </div>
          )}
          ...
          <div className={`${styles.dateTime} flex mb-[15px] items-end`}>
            <div className="flex flex-col">
              ...
              <input type="date" className="min-w-[120px]" {...register('dateMeet', { required: true })} />
            </div>
            <div className="flex flex-col">
              <label className="font-montserrat text-[13px]">Время</label>
              <input type="time" className="min-w-[88px]" {...register('time', { required: true })} />
            </div>

            <div className="flex gap-[4px] items-center">
              <input className="max-w-[50px]" placeholder="минут" {...register('duration')} />
          ...
          <div className="flex flex-col">
            ...
            <select className={`${styles.select} max-w-[200px]`} {...register('formatMeet', { required: true })}>
          ...
          {watchFormat === 'offline' ? (
            <div className="flex flex-col">
              ...
              <input className={styles.input} placeholder="Введите адрес" {...register('location')} />
            </div>
          ) : (
            <div className="flex flex-col">
              <label className="font-montserrat text-[13px]">Место встречи</label>
              <input className={styles.input} placeholder="Введите ссылку" {...register('location')} />
            </div>
          )}
          {watchType === 'client' && (
            <div>
              <label className="font-montserrat text-[13px]">Метод оплаты</label>
              <select className={styles.input} {...register('paymentType')}>
          ...
          {watchType === 'other' && (
            <textarea className={styles.textarea} placeholder="Комментарий" {...register('note')} />
          )}

          <div className={styles.checkboxGroup}>
            <label className="font-montserrat text-[13px]">
              <input type="checkbox" checked={repeatChecked} onChange={() => setRepeatChecked(!repeatChecked)} />
              Повторять
            </label>
            {repeatChecked && (
          ...
          <div className={styles.checkboxGroup}>
            <label className="font-montserrat text-[13px]">
              <input type="checkbox" checked={remindChecked} onChange={() => setRemindChecked(!remindChecked)} />
              Напомнить
            </label>
            {remindChecked && (
